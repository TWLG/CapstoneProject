...
export default function Control_Panel() {
  ...
  // if admin is one of the users roles
  if (
    !session.user?.roles.includes("admin") &&
    !session.user?.roles.includes("control_panel")
  ) {
  ...
  return (
    <div>
      <data value="">
        <p>
          {session
            ? `User: ${session.user?.name},  Role: ${session.user.roles.join(
                ", "
              )}`
      ...
      <div>
        ...
        <div>
          ...
          <form>
            ...
            <div>
              <label>
                Speed (Microseconds Delay 200-4000 **LOWER IS FASTER**):
                <input
                  type="number"
                  min="200"
                  max="4000"
            ...
            <button
              ...
              onClick={() => {
                ...
                const speed = parseInt(
                  (document.getElementById("motorSpeed") as HTMLInputElement)
                    .value
                );
                sendMessage(socket, {
                  type: "setMotorSpeed",
